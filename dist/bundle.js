...
if (accordion) {
  accordion.addEventListener('click', e => {
    ...
    const container = accordionHeader.nextElementSibling; // console.log(accordionHeader.textContent + " is toggled")
...
quoteForm.getElementsByTagName('button')[1];

const validateForm = form => {
  const quoteObject = {}; // TODO: check if form.text && form.author are not empty required)
  // Use a for loop instead of forEach, since `form.elements` is an object, not an array.

  for (let i = 0; i < form.length; i++) {
    const element = form[i]; // If the form element has a name property (which buttons do not) sanitize, trim and add to the object.

    if (element.name) {
      // TODO: Check that at least quote text and author have values
      quoteObject[element.name] = purify.sanitize(element.value.trim());
    }
  }
...
quoteForm.addEventListener('submit', e => {
  ...
  if (form) {
    ...
    sendForm(form);
    quoteForm.nextElementSibling.textContent = `Your quote has been successfully submitted.`; // TODO: reset form here
  } else {
